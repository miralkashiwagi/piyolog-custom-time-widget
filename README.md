...
2. ワークフローが緑色のチェックマーク（成功）になったら、そのワークフローの実行結果ページ下部の**Artifacts** セクションに `piyolog-time-widget-release-apk` というzipがあるのでダウンロードします。中にインストール可能な署名済みの `app-piyolog-time-widget.apk` が入っています。
